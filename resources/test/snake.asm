...
initSnake:
  ...
  lda #$02   ;start direction, put the dec number 2 in register A
  sta $02  ;store value of register A at address $02

  ;initial snake length of 4
  lda #$04   ;start length, put the dec number 4 (the snake is 4 bytes long)
           ;in register A
...
generateApplePosition:
  ...
  adc #$02   ;add to register A, using carry bit for overflow.
...
upKey:
  lda #$04          ;load value 4 into register A, correspoding to the value for DOWN
  ...
  lda #$01          ;Ending up here means the move is legal, load the value 1 (UP) into
                  ;register A
  sta $02         ;Store the value of A (the new direction) into register A
  rts             ;return

rightKey:
  lda #$08          ;load value 8 into register A, corresponding to the value for LEFT
  bit $02         ;AND with current direction at address $02 and check if result
                  ;is zero
  bne illegalMove ;Branch If Not Equal: meaning the zero flag is not set.

  lda #$02          ;Ending up here means the move is legal, load the value 2 (RIGHT) into
                  ;register A
  sta $02         ;Store the value of A (the new direction) into register A
  rts             ;return

downKey:
  lda #$01          ;load value 1 into register A, correspoding to the value for UP
  bit $02         ;AND with current direction at address $02 and check if result
                  ;is zero
  bne illegalMove ;Branch If Not Equal: meaning the zero flag is not set.

  lda #$04          ;Ending up here means the move is legal, load the value 4 (DOWN) into
                  ;register A
  sta $02         ;Store the value of A (the new direction) into register A
  rts             ;return

leftKey:
  lda #$02          ;load value 1 into register A, correspoding to the value for RIGHT
  bit $02         ;AND with current direction at address $02 and check if result
                  ;is zero
  bne illegalMove ;Branch If Not Equal: meaning the zero flag is not set.

  lda #$08          ;Ending up here means the move is legal, load the value 8 (LEFT) into
                  ;register A
  sta $02         ;Store the value of A (the new direction) into register A
  rts             ;return
...
checkSnakeCollision:
  ldx #$02 ;Load the value 2 into the X register, so we start with the first segment
...
upup:
  ...
  lda #$01   ;load hex value $1 (dec 1) into register A
...
downdown:
  ...
  lda #$06   ;load the value hex $6 into the A register
...
drawSnake:
  ldx #$00      ;set the value of the X register to 0
  lda #$01      ;set the value of the A register to 1
  ...
  lda #$00      ;set the value of the a register to 0
  sta ($10,x) ;dereference to the memory address that's stored at address
...
spinWheels:
  ...
  ldx #$00       ;load zero in the X register
spinloop:
